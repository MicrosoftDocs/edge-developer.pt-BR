...
Se você já sabe como a extensão do Microsoft Edge AppXs é criada, você pode ignorar o [uso do ManifoldJS para a extensão do pacote](./using-manifoldjs-to-package-extensions.md) para saber como usar uma ferramenta baseada em Node.js para fazer tudo isso por você!
...
![imagem da estrutura da pasta. Dentro de minha pasta de extensão está AppxManifest.xml, pasta de extensão e pasta ativos](./../../media/packaging_folder-structure.png)
...
Alguns valores no AppXManifest precisam corresponder àqueles definidos no manifesto JSON. Atualize os seguintes valores no appxmanifest.xml com base em seu manifesto de extensão JSON:
