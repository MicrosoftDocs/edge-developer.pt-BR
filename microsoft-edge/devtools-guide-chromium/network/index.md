...
1.  Para [abrir o devtools][DevToolsOpen] , pressione `Control` + `Shift` + `J` \ (Windows, Linux \) ou `Command` + `Option` + `J` \ (MacOS \).  O painel **console** será aberto.  
...
1.  Clique com o botão direito do mouse no cabeçalho da tabela de log de rede e escolha **domínio**.  O domínio de cada recurso agora é mostrado.  
