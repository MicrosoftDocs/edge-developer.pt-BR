...
Se você estiver procurando maneiras de melhorar o desempenho do carregamento de página, **não** comece com a ferramenta **rede** .  Há muitos tipos de problemas de desempenho de carga que não estão relacionados à atividade de rede.  Comece com o painel auditorias porque ele fornece sugestões direcionadas sobre como melhorar sua página.  Navegue até [otimizar a velocidade do site][DevtoolsSpeedGetStarted].  
...
1.  Para [abrir o devtools][DevToolsOpen] , pressione `Control` + `Shift` + `J` \ (Windows, Linux \) ou `Command` + `Option` + `J` \ (MacOS \).  A ferramenta de **console** será aberta.  
...
1.  Selecione a guia **rede** .  O painel **rede** é aberto.  
    ...
    :::image type="complex" source="../media/network-glitch-network-bottom.msft.png" alt-text="Ferramenta de console no DevTools encaixada na parte inferior da janela" lightbox="../media/network-glitch-network-bottom.msft.png":::
       Ferramenta de **console** no devtools encaixada na parte inferior da janela  
    :::image-end:::  
...
1.  Passe o cursor do mouse sobre o cabeçalho da tabela de log de rede, abra o menu contextual \ (clique com o botão direito do mouse \) e escolha **domínio**.  O domínio de cada recurso agora é mostrado.  
...
1.  Escolha o menu suspenso **regulagem** , que é definido como **online** por padrão.  
...
1.  Use o **recarregamento** de longa duração \ ( ![ recarregar ][ImageRefreshIcon] \) e escolha **esvaziar o cache e o recarregamento físico**.  
...
1.  Escolha \ ( ![ configurações de rede ][ImageSettingsIcon] \) e ative a caixa de seleção captura de **tela de captura** .
1.  Atualize a página novamente usando o fluxo de trabalho de **cache e recarregamento de disco vazio** .  Navegue para [simular uma conexão mais lenta](#simulate-a-slower-network-connection) se você precisar de um lembrete sobre como fazer isso.  
...
1.  Escolha a primeira miniatura.  O DevTools mostra o que a atividade de rede estava ocorrendo nesse momento.  
...
1.  Escolha \ ( ![ configurações de rede ][ImageSettingsIcon] \) novamente e desative a caixa de seleção captura de **tela de captura** para fechar o painel capturas de tela.
...
1.  Escolher `getstarted.html` .  A guia **cabeçalhos** é mostrada.  Use esta guia para inspecionar cabeçalhos HTTP.  
...
1.  Escolha a guia **Visualizar** .  Uma renderização básica do HTML é mostrada.  
    ...
    A guia é útil quando uma API retorna um código de erro em HTML.  Pode ser mais fácil ler o HTML renderizado do que o código-fonte HTML ou ao inspecionar imagens.  

1.  Escolha a guia **resposta** .  O código-fonte HTML é mostrado.  
    ...
    > [!TIP]
    > Quando um arquivo é minified, escolha o **formato** \ ( ![ formato ][ImageFormatIcon] \) na parte inferior da guia **resposta** para reformatar o conteúdo do arquivo para facilitar a leitura.  
    
1.  Escolha a guia **intervalo** .  Uma divisão da atividade de rede do recurso será exibida.  
...
1.  Escolha **Pesquisar** \ ( ![ localizar ][ImageSearchIcon] \).  O painel Pesquisar é aberto à esquerda do log de rede.  
...
1.  Digite `Cache-Control` e selecione `Enter` .  O painel Pesquisar lista todas as ocorrências `Cache-Control` que ele encontra em cabeçalhos ou conteúdo do recurso.  
...
A barra de ferramentas **filtros** deve ser ativada por padrão.  Se não:  

1.  Escolha **filtro** \ ( ![ filtro ][ImageFilterIcon] \) para mostrá-lo.  
...
1.  Para ver também os scripts, segure `Control` \ (Windows, Linux \) ou `Command` \ (MacOS \) e escolha **js**.  
...
1.  Escolha **tudo** para remover os filtros e ver todos os recursos novamente.  
...
1.  Tipo `block` , escolha **Mostrar bloqueio de solicitação**e selecionar `Enter` .  
...
Navegue até a [referência de rede][DevtoolsNetworkReference] para descobrir recursos mais devtools relacionados à atividade de inspeção de rede.  
