...
1.  Clique com o botão direito do mouse no `<li>Magritte</li>` nó e escolha **rolar para o modo de exibição**.  O visor rola novamente para que você possa ver o nó **Magritte** .  Consulte [Apêndice: opções ausentes](#appendix-missing-options) se você não conseguir ver a opção **rolar para a exibição** .
...
1.  Em **forçar estado**, passe o mouse sobre **o Lord dos voa**.  A cor do plano de fundo se torna laranja.  
    ...
    1.  Clique com o botão direito do mouse `<li class="demo--hover">The Lord of the Flies</li>` e escolha **forçar estado**  >  **: focalizar**.  Consulte [Apêndice: opções ausentes](#appendix-missing-options) se você não vir essa opção.  A cor do plano de fundo permanece laranja, mesmo que você não esteja efetivamente passando o mouse sobre o nó.  
...
1.  Em **armazenar como variável global**, escolha com o botão direito **do mouse na grande suspensão** e escolha **inspecionar**.  
    1.  Clique com o botão direito do mouse `<li>The Big Sleep</li>` na árvore DOM e escolha **armazenar como variável global**.  Consulte [Apêndice: opções ausentes](#appendix-missing-options) se você não vir essa opção.  
...
1.  Em **interromper as modificações de atributo**, escolha **sauerkraut** e escolha **inspecionar**.  
    1.  Na árvore DOM, clique com o botão direito do mouse `<li id="target">Sauerkraut</li>` e escolha **interromper nas**  >  **modificações de atributo**.  Consulte [Apêndice: opções ausentes](#appendix-missing-options) se você não conseguir ver esta opção.
...
1.  Em **interromper a remoção do nó**, escolha **Neuromancer** e escolha **inspecionar**.  
    1.  Na árvore DOM, clique com o botão direito do mouse `<li id="target">Neuromancer</li>` e escolha **interromper na**  >  **remoção de nó**.  Consulte [Apêndice: opções ausentes](#appendix-missing-options) se você não conseguir ver esta opção.  
...
1.  Em **interromper modificações de subárvore**, escolha **uma incêndio na profundidade** e escolha **inspecionar**.  
    1.  Na árvore DOM, clique com o botão direito do mouse `<ul id="target">` , que é o nó acima `<li>A Fire Upon the Deep</li>` e escolha **interromper**  >  **alterações de subárvore**.  Consulte [Apêndice: opções ausentes](#appendix-missing-options) se você não conseguir ver esta opção.  
