...
Esta página é uma referência de atalhos de teclado no Microsoft Edge DevTools.

Você também pode encontrar atalhos nas dicas de ferramentas. Passe o mouse sobre um elemento de interface do usuário do DevTools para exibir sua dica de ferramenta. Se o elemento tiver um atalho, a dica de ferramenta o incluirá.

## Atalhos de teclado para abrir o DevTools   

Para abrir o DevTools, pressione os seguintes atalhos de teclado enquanto o cursor estiver focalizado no visor do navegador:
...
| Alternar [dispositivomode][DevtoolsDeviceModeIndex] | `Control`+`Shift`+`M` | `Command`+`Shift`+`M` |  
...
| Executar trecho | Pressione `Control` + `O` para abrir o [menu de comando][DevtoolsCommandMenuIndex], digite `!` seguido do nome do script e pressione `Enter` | Pressione `Command` + `O` para abrir o [menu de comando][DevtoolsCommandMenuIndex], digite `!` seguido do nome do script e pressione `Enter` |  
...
| Recolha o nó atualmente selecionado. Se o nó já estiver recolhido, esse atalho selecionará o elemento acima dele | `Left Arrow` | `Left Arrow` |  
| Expandir ou recolher o nó selecionado atualmente e todos os seus filhos | Segure `Control` + `Alt` e clique no ícone de seta ao lado do nome do elemento | Segure `Option` e clique no ícone de seta ao lado do nome do elemento |  
...
| Ir para um número de linha do arquivo aberto no momento | Pressione `Control` + `O` para abrir o [menu de comando][DevtoolsCommandMenuIndex], digite `:` seguido do número da linha e pressione `Enter` | Pressione `Command` + `O` para abrir o [menu de comando][DevtoolsCommandMenuIndex], digite `:` seguido do número da linha e pressione `Enter` |  
| Vá para uma coluna do arquivo aberto no momento \ (por exemplo, linha 5, coluna 9 \) | Pressione `Control` + `O` para abrir o [menu de comando][DevtoolsCommandMenuIndex], digite `:` , o número da linha, depois `:` o número da coluna e, em seguida, pressione `Enter` | Pressione `Command` + `O` para abrir o [menu de comando][DevtoolsCommandMenuIndex], digite `:` , o número da linha, depois `:` o número da coluna e, em seguida, pressione `Enter` |  
| Vá para uma declaração de função \ (se o arquivo aberto no momento for HTML ou um script \) ou um conjunto de regras \ (se o arquivo aberto no momento for uma folha de estilos \) | Pressione `Control` + `Shift` + `O` , digite o nome da declaração/conjunto de regras ou selecione-o na lista de opções | Pressione `Command` + `Shift` + `O` , digite o nome da declaração/conjunto de regras ou selecione-o na lista de opções |  
