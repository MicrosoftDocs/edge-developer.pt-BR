...
---
# <a name="sources-tool-overview"></a>Visão geral da ferramenta Sources  
...
Para maximizar o tamanho da ferramenta Sources, desfaça DevTools em uma janela separada e, opcionalmente, mova a janela DevTools para um monitor separado.  Consulte [Alterar o posicionamento do Microsoft Edge DevTools (Undock, Dock to bottom, Dock to left)][DevToolsCustomizePlacement].
...
Para acessar quaisquer guias ocultas do painel Navegador, selecione ![ Mais guias ( Mais ](../media/more-tabs-icon.msft.png) **guias**).
...
*   [Usando a guia Scripts de conteúdo para extensões do Microsoft Edge](#using-the-content-scripts-tab-for-microsoft-edge-extensions)
...
*   O **ícone de** nuvem representa uma [origem][HtmlstandardOrigin].  
...
Use a **guia Substituições** do painel **Navegador** para substituir ativos de página (como imagens) por arquivos de uma pasta local.
...
*   [Substituir recursos de página da Web com cópias locais usando o Microsoft Edge DevTools][DevtoolsJavascriptOverrides]

*   [Mapear código pré-processador para código-fonte][DevToolsJavaScriptSourceMaps]

### <a name="using-the-content-scripts-tab-for-microsoft-edge-extensions"></a>Usando a guia Scripts de conteúdo para extensões do Microsoft Edge

Use a **guia Scripts de** conteúdo do painel **Navegador** para exibir quaisquer scripts de conteúdo carregados por uma extensão do Microsoft Edge instalada. 
...
*   [Scripts de conteúdo][MDNContentScripts]
...
*   [Executar trechos de código do JavaScript em qualquer página da Web com o Microsoft Edge DevTools][DevtoolsGuideChromiumJavascriptSnippets]
...
*   Em qualquer lugar no DevTools, selecione `Ctrl` + `P` no Windows/Linux `Command` + `P` ou no macOS.  O Menu de Comando é exibido e lista todos os recursos que estão nas guias do painel **Navegador** da **ferramenta Sources.**  
...
Para que as alterações do JavaScript entre em vigor, selecione `Ctrl` + `S` \(Windows, Linux\) `Command` + `S` ou \(macOS\).  

Se você alterar um arquivo, um asterisco será exibido ao lado do nome do arquivo.
*   Para salvar alterações, selecione `Ctrl` + `S` no Windows/Linux ou `Command` + `S` no macOS.
*   Para desfazer uma alteração, selecione `Ctrl` + `Z` no Windows/Linux ou `Command` + `Z` no macOS.
...
Para encontrar texto no arquivo atual, selecione o painel **Editor** para dar foco a ele e selecione no `Ctrl` + `F` Windows/Linux ou `Command` + `F` no macOS.  
...
:::image type="complex" source="../media/pretty-printed.msft.png" alt-text="Um arquivo JavaScript bastante impresso (reformatado)" lightbox="../media/pretty-printed.msft.png":::
   Um arquivo JavaScript bastante impresso (reformatado)  
:::image-end:::  
...
Em um ambiente de desenvolvimento, seu servidor pode incluir seus mapas de origem e seus arquivos `.ts` originais `.jsx` ou para React.  A **ferramenta Sources** exibe esses arquivos, mas não permite que você edite esses arquivos.  Quando você definir pontos de interrupção e usar o depurador, o DevTools exibe seus arquivos originais ou, na verdade, passo a passo pela versão minificada de seus `.ts` `.jsx` arquivos JavaScript.
...
Para salvar alterações, selecione `Ctrl` + `S` no Windows/Linux ou `Command` + `S` no macOS.  Um arquivo editado é marcado por um asterisco.  

Para encontrar texto, selecione `Ctrl` + `F` no Windows/Linux ou `Command` + `F` no macOS.

Para desfazer uma edição, selecione `Ctrl` + `Z` no Windows/Linux ou `Command` + `Z` no macOS.
...
1.   No painel **Navegador,** selecione as releitos (...) (**Mais opções**) e selecione Abrir **Arquivo**.  O Menu de Comando é exibido.  
...
Use o depurador JavaScript para passar pelo código JavaScript retornado pelo servidor. O depurador inclui o painel **Depurador,** juntamente com pontos de interrupção que você definir em linhas de código no **painel Editor.**
...
O depurador no DevTools foi projetado para parecer, parecer e trabalhar como o [depurador][CodeVisualStudioComDocsEditorDebugging] em Visual Studio Code e o [depurador no Visual Studio][DMCVisualStudioDebuggerNavigatingThroughCodeWithTheDebugger].
...
*   [Depurar diretamente Visual Studio Código](#debug-from-visual-studio-code-directly)
...
Para solucionar problemas de código JavaScript, você pode inserir `console.log()` instruções no **painel Editor.**  Outra abordagem mais poderosa é usar o depurador do Microsoft Edge DevTools.  O uso de um depurador pode ser mais simples do que , depois que você estiver familiarizado com a `console.log()` abordagem do depurador.
...
1.  Abra a página da Web em uma nova guia do navegador.  Por exemplo, abra essa página da Web de formulário em uma nova guia: [Demonstração: Começar a depurar JavaScript com o Microsoft Edge (Chromium) DevTools][DevtoolsGlitchMeDebugJsGetStarted].
...
### <a name="debug-from-visual-studio-code-directly"></a>Depurar diretamente Visual Studio Código

Para usar o depurador mais completo do código Visual Studio em vez do depurador DevTools, use a extensão Ferramentas do Microsoft Edge para **VS Code** para Visual Studio Code.
...
:::image type="complex" source="../media/microsoft-edge-tools-for-vs-code-extension.msft.png" alt-text="A extensão microsoft edge tools for VS Code for Visual Studio Code" lightbox="../media/microsoft-edge-tools-for-vs-code-extension.msft.png":::
   A **extensão ferramentas do Microsoft Edge para VS Code** para Visual Studio Código  
:::image-end:::  
...
Para obter mais informações, [consulte Visual Studio Visão][DevToolsVSCodeIndex] geral do código e a página Leitura do GitHub, Ferramentas de Desenvolvedor do Microsoft Edge para Visual Studio [Código][GithubMicrosoftVscodeEdgeDevtools].
...
*   [Introdução à depuração de JavaScript no Microsoft Edge DevTools][DevtoolsGuideChromiumJavascriptIndex] - Um tutorial (com capturas de tela), usando um projeto existente e simples.
