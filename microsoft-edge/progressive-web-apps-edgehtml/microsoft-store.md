...
3. Quando você carrega pacotes do aplicativo, os valores *DisplayName*, *Name*, *Publisher*e *PublisherDisplayName* em seu arquivo *. appxmanifest* devem coincidir com os valores atribuídos ao seu aplicativo no painel do centro de desenvolvimento do Windows, ao reservar o nome. 
    ...
    Faça logon no [painel do centro de desenvolvimento do Windows](https://developer.microsoft.com/dashboard/windows/overview)e localize os valores de identidade do aplicativo na identidade do aplicativo de **Gerenciamento do aplicativo**  >  **App identity**:
...
- [X] um arquivo de manifesto de aplicativo Web não vazio, com pelo menos:

  - Nome
  - Descrição
  - Ícone do aplicativo com pelo menos 512 x 512 pixels

- [X] lógica de núcleo exclusiva (não há código modificado minimamente a partir de um modelo de código [clichê do aplicativo](https://en.wikipedia.org/wiki/Boilerplate_code) )

- [X] ser servido por meio de uma conexão segura (HTTPS)

- [X] script (s) de trabalho do serviço

- [X] não violar as leis ou [políticas da Microsoft Store](/legal/windows/agreements/store-policies).
