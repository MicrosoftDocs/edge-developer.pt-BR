...
---
# Protocolo de DevTools Microsoft Edge (Chromium)

Com o turno na plataforma da Web subjacente do Microsoft Edge com o Chromium, o [protocolo de devtools do Microsoft Edge (EdgeHTML)](./devtools-protocol/index.md) não receberá mais atualizações. O protocolo do DevTools do Microsoft Edge (Chromium) corresponderá às APIs do protocolo Chrome DevTools que continuarão no futuro. 
...
> [!NOTE]
> Qualquer método que tenha sido prefixado `ms` no [protocolo devtools Microsoft Edge (EdgeHTML)](./devtools-protocol/index.md) não tem mais suporte no protocolo devtools Microsoft Edge (Chromium).
...
2. Inicie o Microsoft Edge (Chromium) com a porta de depuração remota:

    ```
    msedge.exe --remote-debugging-port=9222
    ```

3. Opcionalmente, você pode iniciar uma instância separada do Edge usando um perfil de usuário distinto, se desejar:

    ```
    msedge.exe --user-data-dir=<some directory>
    ```

4. Em seguida, use o `list` ponto de extremidade http para obter uma lista de destinos de página anexáveis:

    ```
    http://localhost:9222/json/list
    ```

5. Por fim, conecte-se aos `webSocketDebuggerUrl` comandos de destino e problema desejados/assine mensagens de evento por meio do devtools Web Socket Server.
...
Para saber como configurar seu dispositivo Windows 10 e conectar-se a ele a partir de seu computador de desenvolvimento, confira [este tutorial](./devtools-guide-chromium/remote-debugging/windows.md).
...
Fornece uma lista de candidatos a todos os `msedge.exe` processos (incluindo [PWAs](./progressive-web-apps-chromium/index.md) e todas as guias em todas as instâncias do Microsoft Edge) no dispositivo Windows 10 para depuração.
